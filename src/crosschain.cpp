...
uint256 CalculateProofRoot(const char* symbol, uint32_t targetCCid, int kmdHeight,
        std::vector<uint256> &moms, uint256 &destNotarisationTxid, bool &MoMoMpossible)
{
    /*
    ...
    for (int i=0; i<NOTARISATION_SCAN_LIMIT_BLOCKS; i++) {
        ...
        // that is not the first 3 nota on the chain!
        // MoMoMpossible needs to be somehow set to true after 3 notarizations have happened for a chain. 
        // we need to call scanNotarisationsDB here and check that the last nota has MoMoMpossible set.
        // if its not set then keep scanning back. If there are not 3 results, set it to false, if there are set to true. 
        BOOST_FOREACH(Notarisation& nota, notarisations) {
            if ( nota.second.MoMoMpossible && strcmp(nota.second.symbol, symbol) == 0 )
            {
        ...
        const TxProof assetChainProof)
{
    ...
    uint256 MoMoM = CalculateProofRoot(targetSymbol, targetCCid, kmdHeight, moms, targetChainNotarisationTxid, MoMoMpossible);
    if (MoMoM.IsNull())
