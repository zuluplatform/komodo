...
extern int32_t KOMODO_LONGESTCHAIN,ASSETCHAINS_STREAM;
...
UniValue getdatafromblock(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "  \"firstdeqid\" : n,            (numeric) The sequence id of the first data chunk in this block\n"
            "  \"lastseqid\" : n,             (numeric) The sequence id of the last data chunk in this block\n"
            + HelpExampleCli("getdatafromblock", "\"00000000c937983704a73af28acdec37b049d214adbda81d7e2a3dd146f6ed09 false\"")
            + HelpExampleRpc("getdatafromblock", "\"00000000c937983704a73af28acdec37b049d214adbda81d7e2a3dd146f6ed09\"")
            + HelpExampleCli("getdatafromblock", "12800")
            + HelpExampleRpc("getdatafromblock", "12800 false")
        );

    if ( ASSETCHAINS_STREAM == 0 )
        throw runtime_error("This chain is not a streamer chain!");
    ...
    UniValue result(UniValue::VOBJ);
    int firstseqid = 0,lastseqid = 0,i = 0,did1 = 0,skippedtxs = 0,failed = 0,getfirstblock = 0;
    static std::string streamid,firsttxid;
    static int firsttxnHeight;
    std::string blockdata;
    fprintf(stderr, "number of tx in block: %ld\n", block.vtx.size());
    // If block tx size is > 2 then we can do this
    if ( block.vtx.size() > 2 )
    {
